...
const { callLLM, getPrimaryProvider } = require('../llm/llmClient');
const {
	registerAgent,
	getStatus,
	getHome,
	...
	upvoteComment,
	followAgent,
} = require('../integrations/moltbookClient');
const {
	logFeedFetch,
	logHomeFetch,
	logCommentsFetch,
...
// personalize.json sekarang di folder data/ pada root project.
...
function loadPersonalize() {
	try {
		...
		console.warn(
			'personalize.json not found or invalid, using defaults:',
			err.message,
		);
		return {
			agent: {
				name: 'MoltbookAgent',
				description:
					'An agentic AI on Moltbook. Edit personalize.json to set your agent name and prompts.',
			},
			keywords: { music: [] },
			prompts: {},
		};
...
const PREFERRED_KEYWORDS = Array.isArray(personalizeConfig.keywords?.music)
	? personalizeConfig.keywords.music
	: [];
...
function recordVerificationAttempt(entry) {
	try {
		const state = loadState();
		const history = Array.isArray(state.verificationHistory)
			? state.verificationHistory
			: [];
		const next = [{ ...entry, at: new Date().toISOString() }, ...history].slice(
			0,
			20,
		);
...
async function solveAndVerifyVerification(state, verification, logLabel) {
	const { verification_code: verificationCode, challenge_text: challengeText } =
		verification;
		...
		const historyBlock =
			failed.length > 0
				? `\nYou previously got these wrong — do not repeat the same answer:\n${failed
						.map(
							(e) =>
								`- Challenge: "${(e.challengeText || '').slice(0, 120)}" → you answered "${e.ourAnswer}" (incorrect).`,
						)
						.join('\n')}\n`
				: '';

		const solvePrompt = fillTemplate(
			personalizeConfig.prompts.verification ||
	...
	try {
		...
		console.log(
			`${logLabel}:`,
			verifyResult?.message || safeStringify(verifyResult),
		);
...
async function generateCommentForPost(state, targetPost, fallbackContent) {
	...
	const agentName =
		personalizeConfig.agent?.name || state.agentName || 'MoltbookAgent';
	const noMusicRule = isMusicPost
		? ''
		: '\n' +
			(
				personalizeConfig.prompts.commentNoMusicRule ||
				'CRITICAL: Respond only about the topic of this post. Do not bring up unrelated topics.'
			).trim() +
			'\n';
	const prompt = fillTemplate(
		personalizeConfig.prompts.comment ||
			'You are {{agentName}}. Post: {{title}} {{body}}. {{noMusicRule}} Persona: {{persona}}. Respond with ONLY the comment text.',
		{
			agentName,
			title,
			body: body || '(no body)',
...
async function classifyInteraction({ postTitle, postContent, commentText }) {
	...
	const instruction = fillTemplate(
		personalizeConfig.prompts.classify ||
			'Classify topic for {{agentName}}. Post: {{postTitle}} {{postContent}} Comment: {{commentText}}. Respond JSON: {"topic":"...","subtopics":[],"sentiment":"..."}',
		{
			agentName,
			postTitle: postTitle || '-',
...
async function maybeUpdatePersonaSummary() {
	try {
		...
		if (
			lastUpdate &&
			now - lastUpdate < 60 * 60 * 1000 &&
			topics.length <= lastCount + 3
		) {
			return state;
		}
		...
		const agentName = personalizeConfig.agent?.name || 'MoltbookAgent';
		const prompt = fillTemplate(
			personalizeConfig.prompts.personaSummary ||
				'Summarize personality of {{agentName}}. Topics: {{topics}} Actions: {{actions}}. Respond JSON: {"summary":"...","bullets":[]}',
			{
				agentName,
...
function pickFollowCandidate(state) {
	const selfName =
		state.agentName || personalizeConfig.agent?.name || 'MoltbookAgent';
	const following = new Set(
		Array.isArray(state.followingNames) ? state.followingNames : [],
	);
	const engagement = getEngagementByAuthor(state);
	const candidates = Object.entries(engagement)
		.filter(
			([name]) =>
				name !== selfName && !following.has(name) && engagement[name] >= 2,
		)
		.sort((a, b) => b[1] - a[1]);
...
/**
 * Build a single-thread summary of all comments in a post for reply context.
 * Uses only this post's conversation so we avoid cross-context (post A vs post B).
 * @param {Array<{ author?: { name?: string }, content?: string, created_at?: string }>} comments
 * @param {string} selfName - Our agent name so we label our own comments as "You".
 * @returns {string} Formatted thread text, or fallback message if empty.
 */
function buildPostThread(comments, selfName) {
	if (!Array.isArray(comments) || comments.length === 0) {
		return '(No prior conversation in this post)';
	}
	const sorted = [...comments].sort((a, b) => {
		const atA = a.created_at || a.createdAt || '';
		const atB = b.created_at || b.createdAt || '';
		return atA.localeCompare(atB);
	});
	const lines = sorted.map((c) => {
		const author = c.author?.name || c.author_name || 'Unknown';
		const label = author === selfName ? 'You' : author;
		const content = (c.content || c.body || '').trim() || '(no text)';
...
function applyActionHeuristics(decision, stats, state) {
	...
	const lastCommentAt = state.lastCommentAt
...
async function decideNextAction(state) {
	...
	const topicHistory = Array.isArray(state.topicHistory)
		? state.topicHistory.slice(0, 15)
		: [];
	...
	const canFollow =
		followCandidate &&
		(!lastFollowAt ||
			Date.now() - new Date(lastFollowAt).getTime() > sixHoursMs);
	...
	const context = {
		...
		recentTopics: topicHistory.map((t) => ({
			topic: t.topic,
		...
		recentActionsSummary: {
			...
			lastActions: recent.map((a) => ({
				...
				at: a.at,
				summary: a.summary,
				postTitle: a.postTitle || null,
				postAuthor: a.postAuthor || null,
				targetAuthor: a.targetAuthor || null,
				topic: a.topic || null,
	...
	const agentName =
		state.agentName || personalizeConfig.agent?.name || 'MoltbookAgent';
	const instruction = fillTemplate(
		personalizeConfig.prompts.decideNextAction ||
			'You are {{agentName}}. Current state: {{context}}. Decide one action and delaySeconds. Respond with a single JSON object only.',
		{
			agentName,
...
async function runAction(decision, state) {
	...
	if (action === 'register') {
		...
		const defaultDesc =
			personalizeConfig.agent?.description ||
			'An agentic AI on Moltbook. Edit personalize.json to set your description.';
		const agentName =
			state.agentName ||
			(typeof decision.agentName === 'string' &&
			decision.agentName.trim().length > 0
				? decision.agentName.trim()
				: defaultName);
		const description =
			state.agentDescription ||
			(typeof decision.description === 'string' &&
			decision.description.trim().length > 0
		...
		if (!apiKey) {
			throw new Error(
				'Register succeeded but no api_key was returned in the response.',
			);
		}
	...
	if (action === 'home') {
		...
		const selfName =
			data?.your_account?.name ||
			state.agentName ||
			personalizeConfig.agent?.name ||
			'MoltbookAgent';
		logHomeFetch(selfName, data);
		...
		recordAction('home', `Home: karma=${karma}, unread=${unread}`, {
			karma,
	...
	if (action === 'follow') {
		...
		const sixHoursMs = 6 * 60 * 60 * 1000;
		const lastFollowAt = state.lastFollowAt
			? new Date(state.lastFollowAt).getTime()
			: 0;
		...
		try {
			...
			recordAction(
				'follow',
				`Followed ${name} (consistently enjoyed their content).`,
				{
					agentName: name,
				},
			);
	...
	if (action === 'post') {
		...
		const selfNamePost =
			state.agentName || personalizeConfig.agent?.name || 'MoltbookAgent';
		logPostCreated(selfNamePost, post, title, content, 'general');
		...
		if (
			...
			await solveAndVerifyVerification(
				state,
				verification,
	...
	if (action === 'comment') {
		...
		const feed = await getFeed(state.moltbookApiKey, {
			sort: 'hot',
			limit: 20,
		});
		...
		const selfNameFeed =
			state.agentName || personalizeConfig.agent?.name || 'MoltbookAgent';
		logFeedFetch(selfNameFeed, feed, 'hot', 20);
		...
		const pickWithVariety = () => {
			const preferredInPool = pool.filter((p) => {
				const text =
					`${p.title || ''} ${p.content_preview || ''} ${p.content || ''}`.toLowerCase();
		...
		const content = await generateCommentForPost(
			state,
			target,
			fallbackContent,
		);
		...
		console.log(
			`Adding comment to post: ${target.title || target.post_title || target.post_id}`,
		);
		...
		const selfNameComment =
			state.agentName || personalizeConfig.agent?.name || 'MoltbookAgent';
		logCommentAdded(selfNameComment, target, content);
	...
	console.log(
		`Aksi tidak dikenal dari Gemini: ${action}, tidak melakukan apa-apa.`,
	);
	return state;
}
...
async function maybeReplyToComments(state) {
	...
	logCommentsFetch(selfName, postId, targetActivity.post_title, comments);

	// Ambil komentar terbaru yang bukan dari diri sendiri.
	const targetComment =
		comments.find((c) => c.author?.name !== selfName) || comments[0];

	// History hanya dari post ini (semua komentar + reply kita di post ini), bukan cross-context dari post lain.
	const postThread = buildPostThread(
		comments,
		selfName || personalizeConfig.agent?.name || 'MoltbookAgent',
	);

	const postContent =
		targetActivity.preview ||
		(Array.isArray(state.topicHistory)
			? state.topicHistory.find(
					(t) =>
						t.source === 'post' &&
						t.postTitle &&
						targetActivity.post_title &&
						t.postTitle === targetActivity.post_title,
				)?.snippet || null
			: null);

	const agentName = personalizeConfig.agent?.name || 'MoltbookAgent';
	const decisionPrompt = fillTemplate(
		personalizeConfig.prompts.replyToComment ||
			'You are {{agentName}}. Your name: {{selfName}}. Post: {{postTitle}}. Post content: {{postContent}}. Conversation in this post only:\n{{postThread}}\n\nTarget comment: {{targetComment}}. Decide shouldReply and reply. Respond JSON only.',
		{
			agentName,
			selfName: selfName || 'MoltbookAgent',
			postTitle: targetActivity.post_title,
			postContent: postContent || '(no post body available)',
			postThread,
	...
	if (
		!replyDecision ||
		replyDecision.shouldReply !== true ||
		!replyDecision.reply
	) {
	...
	const verification =
		commentResult?.verification || commentResult?.comment?.verification;
	if (
		verification &&
		verification.verification_code &&
		verification.challenge_text
		...
		console.log(
			'Reply comment requires verification. Solving the challenge...',
		);
		await solveAndVerifyVerification(
			state,
			verification,
			'Reply comment verification result',
		);
	}
